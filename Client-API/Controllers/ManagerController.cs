...
namespace Client_API.Controllers
{
    ...
    public class ManagerController : ControllerBase
    {
        ...
        public ManagerController(IHostEnvironment webHostEnvironment, IAccountService acc, IManagerService manager, IRoleService role, ITokenService token, IHashService hash, IImageSetService imgSet, IImageService image)
        {
        ...
        public async Task<IActionResult> CheckToken()
        {
            ...
            if (!header.IsNullOrEmpty())
            {
                ...
                if (!token.IsNullOrEmpty())
                {
                    ...
                    if (result != null && result.RoleName == "Manager")
                    {
                        return Ok(result);
                    }
        ...
        {
            Role roleCheck = await _role.GetByNameAsync("Manager");
        ...
        [Authorize(Roles = "Manager")]
        [HttpPost("Create")]
        public async Task<IActionResult> Create([FromBody] CreateManagerViewModel createInfor)
        {
            string header = Request.Headers["Authorization"].ToString();
            if (header != null && header != string.Empty)
            {
                ...
                if (token != null)
                {
                    ...
                    if (checkedToken != null && checkedToken.RoleName == "Manager")
                    {
                        var emailValidate = await _manager.CheckMailExistAsync(createInfor.Email);
                        if (emailValidate?.IsSuccess == false)
                            return StatusCode(emailValidate.HttpResponse.StatusCode, emailValidate);
                        Role role = await _role.GetByNameAsync("Manager");
                        if (role != null)
                        {
                            ...
                            if (accountCreated.IsSuccess)
                            {
                                ...
                                if (imgsetCreated.IsSuccess)
                                {
                                    ...
                                    //Create Manager
                                    var result = await _manager.CreateAsync(createInfor, checkedToken.AccountId);
        ...
        public async Task<IActionResult> Get(string accId)
        {
            ...
            if (header != null && header != string.Empty)
            {
                ...
                if (token != null)
                {
                    ...
                    if (checkedToken != null && checkedToken.RoleName == "Manager")
                    {
                        Manager manager = await _manager.CheckManagerExistAsync(accId);
                        if(manager != null)
                        {
                            var result = await _manager.GetByAccIdAsync(accId);
                            if (result.IsSuccess)
                            {
        ...
        public async Task<IActionResult> GetAll(string accId)
        {
            ...
            if (header != null && header != string.Empty)
            {
                ...
                if (token != null)
                {
                    ...
                    if (checkedToken != null && checkedToken.RoleName == "Manager")
                    {
                        var result = await _manager.GetAllByAccIdAsync(accId);
                        if (result.IsSuccess)
                        {
                            return Ok(result.Value);
                        }
                ...
                if (token != null)
                {
                    ...
                    if (checkedToken != null && checkedToken.RoleName == "Manager")
                    {
                        ...
                        foreach (IFormFile image in files)
                        {
                            ...
                            bool imageSaved = await _image.CreateManagerImageAsync(_webHostEnvironment.ContentRootPath, new ImageViewModel()
                            {
                                ImgSetId = imgSetId,
                                ImgUrl = imageUrl
                            });
        ...
        public async Task<IActionResult> Update(string accId)
        {
            ...
            if (header != null && header != string.Empty)
            {
                ...
                if (token != null)
                {
                    ...
                    if (checkedToken != null && checkedToken.RoleName == "Manager")
                    {
                        int result = await _manager.UpdateActiveAsync(manager.AccId);
                        if (result == 200)
                        {
                            return Ok(result);
                        }
        ...
        [Authorize(Roles = "Manager")]
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] UpdateManagerViewModel model)
        {
            string header = Request.Headers["Authorization"].ToString();
            if (header != null && header != string.Empty)
            {
                ...
                if (token != null)
                {
                    ...
                    if (checkedToken != null && checkedToken.RoleName == "Manager")
                    {
                        var result = await _manager.UpdateProfileAsync(model, checkedToken.AccountId);
                        if (result.IsSuccess)
                        {
                            return Ok();
                        }
            ...
            if (header != null && header != string.Empty)
            {
                ...
                if (token != null)
                {
                    ...
                    if (checkedToken != null && checkedToken.RoleName == "Manager")
                    {
                        ...
                        var result = await _manager.ChangePasswordAsync(passwordChange, checkedToken.AccountId);
                        if (result?.IsSuccess == true)
                        {
                            return Ok();
                        }
