...
namespace Client_Employee_Services.Employee
{
    public interface IEmployeeService
    {
        Task<ResponseBase> CreateAsync(CreateEmployeeViewModel model, string token);
        ...
        Task<List<EmployeeViewModel>> GetAllByAccIdAsync(string accountId, string token);
    ...
    public class EmployeeService : IEmployeeService
    {
        ...
        public async Task<ResponseBase> CreateAsync(CreateEmployeeViewModel model, string token)
        {
            try
            {
                _httpClient.DefaultRequestHeaders.Remove("Authorization");
                _httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + token);
                HttpResponseMessage response = await _httpClient.PostAsJsonAsync("/api/Employee/Create", model);
                if (response.IsSuccessStatusCode)
                {
                    return ResponseBase.Result(true, 200);
                }
        ...
        public async Task<ResponseBase> GetBrandId(CreateEmployeeViewModel model)
        {
            try
            {
                _httpClient.DefaultRequestHeaders.Remove("Authorization");
                _httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer ");
                HttpResponseMessage response = await _httpClient.GetAsync($"/api/Brand/ByName/");
                if (response.IsSuccessStatusCode)
                {
                    return ResponseBase.Result(true, 200);
                }
        ...
        public async Task<List<EmployeeViewModel>> GetAllByAccIdAsync(string accountId, string token)
        {
            try
            {
                _httpClient.DefaultRequestHeaders.Remove("Authorization");
                _httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + token);
                HttpResponseMessage response = await _httpClient.GetAsync("/api/Employee/GetAll/" + accountId);
                if (response.IsSuccessStatusCode)
                {
        ...
        public async Task<bool> UpdateActiveAsync(string accId, string token)
        {
            try
            {
                _httpClient.DefaultRequestHeaders.Remove("Authorization");
                _httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + token);
                HttpResponseMessage response = await _httpClient.PutAsync("/api/Employee/Active/" + accId, null);
                if (response.IsSuccessStatusCode)
                {
                    return true;
                }
